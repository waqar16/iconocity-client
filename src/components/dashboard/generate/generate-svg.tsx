...
const GenerateSvg: React.FC<GenerateSvgProps> = ({
  ...
  const result: ProjectSvg[] =
    data?.results?.map((icon: any) => ({
      id: Number(icon.id),
      url: icon.url,
    })) ?? [];
  ...
  const {
    mutate: fetchIconById,
    isLoading: isFetching,
    data: similarIconData,
  } = UseFetchIconById();
  ...
  useEffect(() => {
    if (similarIconData?.similar_icons) {
      setTotalSimilarPages(
        Math.ceil(similarIconData?.similar_icons.length / PAGE_SIZE)
      );
  ...
  return (
    <div className="bg-[#0E142D] border border-[#1C2037] rounded-2xl px-8 py-5 ">
      ...
      <div className="w-full flex flex-row items-center justify-end mb-2">
        <Tooltip
          title="This Section contains your generated icons"
          placement="top"
          sx={{
            tooltip: {
      ...
      <div className="flex flex-col items-center my-2 w-full">
        ...
        <p className="text-gray-500">
          {(isShowingSimilarIcons ? visibleSimilarIcons : visibleIcons).length
            ? "Click on icons to view more options"
          ...
          <div className="w-10/12 grid grid-cols-4 xl:grid-cols-4 gap-4 xl:gap-4 3xl:gap-6">
            {(isShowingSimilarIcons
              ...
              >
                ...
                {showMenu &&
                  activeIcon?.id == (icon.similar_icon_id || icon.id) && ( // Show options if the clicked icon matches the active icon
                    <div
                      ref={menuRef}
                      className={`   flex flex-col items-center w-4 h-4 bg-black rounded-full absolute left-[100%] top-[-190px] z-[4000]`}
                    >
                        ...
                        <button
                          onClick={() => {
                            handleSearchForSimilarIcon(
                              icon.id || icon.similar_icon_id || 0
                            );
                            setShowMenu(false);
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white">
              <strong>
                {`${
                  isShowingSimilarIcons ? similarPageNumber : pageNumber
                } of ${isShowingSimilarIcons ? totalSimilarPages : totalPages}`}
              </strong>
            </span>
            <Button
              onClick={
                isShowingSimilarIcons ? handleSimilarNextBtn : handleNextBtn
              }
              disabled={
                isShowingSimilarIcons
