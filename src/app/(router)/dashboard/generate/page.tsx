  ...
  return (
    <ProjectProvider>
      <div className="relative flex gap-5 h-full  ">
        ...
        <svg onClick={()=>{setShowChat(!showChat)}} className="cursor-pointer w-10 h-auto" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{showChat && <ChatBot  keywords={keywords} setPageNumber={setPageNumber}/>}
</div>
        </div>
        <GenerateLeftSide />
        {/* center*/}
        <div className="h-[calc(100vh-74px)] flex-1 overflow-y-auto hide-scrollbar pb-10">
          <GenerateSvg activeIcon={activeIcon} setActiveIcon={setActiveIcon} pageNumber={pageNumber} setPageNumber={setPageNumber} setEnableVariation={setEnableVariation}/>
